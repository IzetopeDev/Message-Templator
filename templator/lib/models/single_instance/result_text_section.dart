...
class _ResultTextState extends ConsumerState<ResultText> {
  ...
  void _generateText() {
    ActiveTemplateState state = ref.read(activeTemplateProvider);
    ...
    for (var field in template.formFieldConfigs.values) {
      dynamic value = state.formResponses[field.keyword];
    ...
    setState(() {
      _generatedText = appliedTemplateText ?? "No Template Text!";
    });
